...
class AppendRigOperator(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        bpy.ops.wm.append(
            directory=os.path.join(bpy.context.scene.warframe_tools_props.rig_path, "Collection")
            + os.sep,
            filename=self.rig_name,
        )
        bpy.ops.wm.append(
            directory=os.path.join(bpy.context.scene.warframe_tools_props.rig_path, "Text")
            + os.sep,
            filename="Bones Snap",
        )
            ...
            for mod in obj.modifiers:
                if mod.type == "ARMATURE":
                    if mod.object != target_armature:
                        ...
                        updated_count += 1
        special_rigs = {
            "face rig": ["Face Metarig", "MetaRig"],
            "long arm rig": ["Long Arm Metarig", "Long Arm Metarig"],
        }
                ...
                if not face_meta_rig:
                    ...
                    if snap_script:
                        ...
                        try:
                            ...
                            self.report({"ERROR"}, f"Bone Snap failed: {e!s}")
                        bpy.ops.object.select_all(action="DESELECT")
                        face_meta_rig.select_set(True)
                        context.view_layer.objects.active = face_meta_rig
                        try:
                            ...
                            metacol.hide_viewport = True
                            self.report({"ERROR"}, f"Rigify generation failed: {e!s}")
                            return {"CANCELLED"}
                        metacol.hide_viewport = True
                    else:
