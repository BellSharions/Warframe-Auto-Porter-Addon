...
class NormalToHeightOperator(bpy.types.Operator):
    ...
    def execute(self, context):
        normal_img = bpy.data.images.load(
            bpy.context.scene.warframe_tools_props.normal_to_height_path, check_existing=False
        )
        ...
        height_img = bpy.data.images.new(
            name=f"{normal_img.name}_Height", width=w, height=h, alpha=False, float_buffer=True
        )
    ...
    def normal_to_height(self, normal_img, iterations=5000, damping=0.1):
        ...
        """
        normal_px = np.array(normal_img.pixels).reshape(normal_img.size[1], normal_img.size[0], -1)
        ...
        f[1:-1, 1:-1] = 0.5 * (dx[1:-1, 2:] - dx[1:-1, :-2]) + 0.5 * (dy[2:, 1:-1] - dy[:-2, 1:-1])
        ...
        for _ in range(iterations):
            ...
            new_height[1:-1, 1:-1] = (1 - damping) * height[1:-1, 1:-1] + damping * 0.25 * (
                height[1:-1, :-2]
                + height[1:-1, 2:]
                + height[:-2, 1:-1]
                + height[2:, 1:-1]
    ...
    def execute(self, context):
        ...
        if ram_subdivision:
            subdiv_levels = int(
                max(
                    0,
                    math.floor(math.log(ram_amount / len(obj.data.vertices), 4) - 1)
                    if len(obj.data.vertices) <= ram_amount
                    else 0,
                )
            )
        ...
        if optimized and subdiv_levels > 1:
            ...
            self.report(
                {"INFO"}, f"Added optimized subdivision: Simple(1) + Catmull-Clark({detail_levels})"
            )
...
class DeformOperator(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        tex = bpy.data.textures.get(texture_name) or bpy.data.textures.new(
            name=texture_name, type="IMAGE"
        )
        tex.image = img
        tex.use_preview_alpha = True

        modifier_name = f"Displace_{img.name}"
        mod = obj.modifiers.get(modifier_name) or obj.modifiers.new(
            name=modifier_name, type="DISPLACE"
        )
        mod.texture = tex
        mod.strength = 0.01
...
class RunAllOperationsOperator(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        try:
            ...
            self.report({"ERROR"}, f"Operation failed: {e!s}")
            return {"CANCELLED"}
