...
class SetupPathsOperator(bpy.types.Operator):
    ...
    def determine_steps(self):
        ...
        if mode == "IMPORT":
            ...
            if bpy.context.scene.warframe_tools_props.USE_ROOT_LOCATION and (
                self.pathToRig is None or self.pathToRig == ""
            ):
            ...
            if not self.shader_library_path:
                steps.append(
                    ("shader_library_path", "Select Shader Library Folder", "directory", "")
                )
            self.root = bpy.context.scene.warframe_tools_props.root
            if bpy.context.scene.warframe_tools_props.USE_ROOT_LOCATION and (
                self.root is None or self.root == ""
            ):
    ...
    def execute(self, context):
        if self.current_step >= len(self.steps):
            ...
            if self.shader_library_path:
                bpy.context.scene.warframe_tools_props.shader_library_path = (
                    self.shader_library_path
                )
            ...
            if self.normal_to_height_path:
                bpy.context.scene.warframe_tools_props.normal_to_height_path = (
                    self.normal_to_height_path
                )
            if self.root:
